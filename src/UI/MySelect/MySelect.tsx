...
const MySelect: FC<MySelectProps> = memo(({ options }) => {
  ...
  const { selectedOptionNumber, params } = useTypedSelector(
    (state) => state.filter
  );
  ...
  return (
    <div className={classes.MySelect} ref={MySelect}>
      <div className={classes["MySelect__body"]} onClick={handlerButtonClick}>
        <BsFilterLeft />
        <button className={classes["MySelect__btn"]}>
      ...
      <div
        className={
          isVisible
            ? [classes["MySelect__options"], classes["active"]].join(" ")
            : classes["MySelect__options"]
        }
      >
